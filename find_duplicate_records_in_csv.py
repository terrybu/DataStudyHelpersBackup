...
def logInfo( logMessage ):
    ...
    if ( logFile == None ):
        logFile = open("Logs/find_duplicate_records_%s_%s.txt"%(logFileOutputName,formattedStartTime),"w")
...
def findRepeatingRecords(allRows):
    ...
    for row in allRows:
        ...
        if frequencyOfRecordRepeat > 1:
            ...
            recordsFrequencyDictionary[frequencyOfRecordRepeat].append(row["PhoneNumber"])
    ...
    msg = "There are a total of %d records that need to be deduplicated from the input file. With successful deduplication, output result count should be %d rows"%(len(repeatingRecordsArray),len(allRows)-duplicationMathCount)
    logInfo(msg)
    print(msg)
